...
const FAQS = [
  ...
  { q:'Gene lặn cho mẹ có ý nghĩa gì?',
    a:'Giúp phát hiện mẹ có mang một số biến thể gene bệnh di truyền lặn hay không. Nếu mẹ mang biến thể, bác sĩ có thể tư vấn xét nghiệm gene lặn cho bố (giá ưu đãi 1.800.000 VNĐ) để đánh giá nguy cơ di truyền cho con.' },
...
// FAQ schema is now in index.html as static JSON-LD for better SEO

export default function FAQ() {
  const [open, setOpen] = useState(null);
  return (
    <>
      <section id="faq" className="section" style={{ background:'linear-gradient(145deg,#f8fbff,#fdfcf8)' }}
        aria-labelledby="faq-h2">
        <div className="container-main">
          <div className="text-center mb-8">
            <div className="section-eyebrow justify-center">Câu hỏi thường gặp</div>
            <h2 id="faq-h2" className="text-2xl sm:text-3xl font-black text-slate-900">Câu hỏi thường gặp về xét nghiệm NIPT</h2>
          </div>
            ...
            {FAQS.map((f, i) => (
              <div key={i} className="card overflow-hidden">
                <button
                  ...
                  <span className="text-sm font-bold text-slate-800 pr-4 leading-snug">{f.q}</span>
                  <svg className={`w-5 h-5 text-brand-blue flex-shrink-0 mt-0.5 transition-transform duration-200
                    ${open===i?'rotate-180':''}`}
