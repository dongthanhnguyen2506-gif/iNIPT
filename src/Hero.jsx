...
// pos: objectPosition riêng cho từng ảnh để focus đúng vùng người trên mobile
const SLIDES = [
  { img: IMG_B1, alt: 'Mẹ bầu châu Á — iNIPT sàng lọc trước sinh Invivo Lab', caption: 'An toàn · Không xâm lấn',    pos: 'center 18%' },
  { img: IMG_B2, alt: 'Vợ chồng mẹ bầu châu Á — iNIPT 7+ khuyến nghị',        caption: 'Đồng hành cùng gia đình',   pos: 'center 22%' },
  { img: IMG_B4, alt: 'Gia đình mẹ bầu với bé gái — iNIPT+ kèm gene lặn',      caption: 'Từ tuần thai thứ 9',         pos: 'center 20%' },
  { img: IMG_B5, alt: 'Mẹ bầu với giày baby xanh — xét nghiệm NIPT Invivo Lab', caption: 'Kết quả 5–7 ngày',          pos: 'center 35%' },
];
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const ev = (ev) => { const dx = ev.changedTouches[0].clientX - sx; if (dx < -40) next(); if (dx > 40) prev(); };
    const el = document.getElementById('hero-slides');
    if (!el) return;
    el.addEventListener('touchstart', s,  { passive: true });
    el.addEventListener('touchend',   ev, { passive: true });
    return () => { el.removeEventListener('touchstart', s); el.removeEventListener('touchend', ev); };
  }, [next, prev]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:hidden pt-16">
        ...
        {/* Carousel — tăng chiều cao để thấy người rõ hơn */}
        <div
          id="hero-slides"
          className="relative w-full overflow-hidden"
          style={{ height: 'clamp(260px, 68vw, 400px)' }}
        >
          {SLIDES.map((s, i) => (
            <div
              key={i}
              className={`absolute inset-0 transition-opacity duration-500 ${i === cur ? 'opacity-100' : 'opacity-0'}`}
            >
              <img
                src={s.img}
                alt={s.alt}
                className="w-full h-full object-cover"
                style={{ objectPosition: s.pos }}   {/* ← dùng pos riêng từng ảnh */}
          ...
          {[['←', prev, 'left-3'], ['→', next, 'right-3']].map(([label, fn, pos]) => (
            <button
              key={pos}
              onClick={fn}
              className={`absolute ${pos} top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/80
                flex items-center justify-center shadow text-slate-700 font-bold z-10 active:scale-90`}
              aria-label={label === '←' ? 'Ảnh trước' : 'Ảnh sau'}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round"
                  d={label === '←' ? 'M15 19l-7-7 7-7' : 'M9 5l7 7-7 7'} />
              </svg>
            </button>
          ))}
        ...
        <div className="px-4 pt-5 pb-24">
          <div className="inline-flex items-center gap-1.5 bg-white border border-blue-100 rounded-full
            px-3 py-1 text-[11px] font-bold text-brand-blueMid shadow-sm mb-3">
          ...
          <h1 className="text-[25px] font-black text-slate-900 leading-[1.2] mb-3" aria-hidden="true">
            ...
            <span className="text-slate-600 font-bold text-[17px] block mt-1">
          ...
          <div className="flex gap-2.5 mb-5">
            <button onClick={() => scrollTo('#quiz')}
              className="btn-primary flex-1 justify-center text-[14px] py-3">
              🌱 Bắt đầu cùng Bé Mầm
            </button>
            <button onClick={() => scrollTo('#nipt-what')}
              className="btn-outline px-4 py-3 text-[13px]">
              Tìm hiểu
            </button>
          </div>
        ...
        <div className="max-w-5xl mx-auto px-6 w-full">
          <div className="grid lg:grid-cols-2 gap-12 items-center py-12">
            ...
            <div className="animate-fade-up">
              <div className="inline-flex items-center gap-2 bg-white/80 border border-blue-100 rounded-full
                px-4 py-1.5 text-xs font-bold text-brand-blueMid shadow-card mb-5">
              ...
              <h1 className="text-[42px] font-black text-slate-900 leading-[1.18] mb-5">
                ...
                <span className="block text-slate-600 font-bold text-[75%] mt-1">
                  an tâm hơn cho hành trình làm mẹ
                </span>
              </h1>

              <p className="text-lg text-slate-500 leading-relaxed mb-7 max-w-[480px]">
                Trả lời vài câu hỏi ngắn — <strong className="text-brand-blue">Bé Mầm</strong>{' '}
                gợi ý nhóm gói iNIPT phù hợp nhất với tuần thai và câu chuyện của mẹ.
              </p>
              ...
              <div className="grid grid-cols-4 gap-2.5">
                {TRUST.map(t => (
                  <div key={t.line1}
                    className="bg-white/70 border border-blue-100 rounded-xl px-3 py-2.5
                      text-center shadow-sm hover:shadow-card transition-all">
